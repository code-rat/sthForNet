...
enum  LOG_LEVEL{
    ...
    LOG_LEVEL_FATAL,        //FATAL级别的日志会让程序输出日志后退出
    LOG_LEVEL_CRITICAL      //CRITICAL日志不受日志级别控制，总是输出
};

//提供几个输出级别
//注意：如果输出的日志信息有中文，则格式化字符串需要用_T()宏包裹起来
...
{
public:
    static bool init(const char* pszLogFileName=nullptr,bool bTruncaateLongLine=false,int64_t nRollSize=10*1024*1024);
    static void uninit();
...
private:
    ...
    CAsyncLog& operator=(const CAsyncLog& ths)=delete;
    ...
    static char* fromLog(int index,char* szbuf,size_t size_buf,unsigned char* buffer,size_t size);
